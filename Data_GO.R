...
for(i in 1:N)
{
  ...
  M[i,]<-scale(Mraw[i,]-cbind(rep(1,ntp),Z1[i,])%*%fit.m$coefficients[c(1,3)],center=TRUE,scale=TRUE)
  
  fit.y<-lm(Yraw[i,]~Z0[i,]+Z1[i,])
  Y[i,]<-scale(Yraw[i,]-cbind(rep(1,ntp),Z1[i,])%*%fit.y$coefficients[c(1,3)],center=TRUE,scale=TRUE)
}
Z<-Z0
...
save(list=c("sub.ID","N","ntp","TR","Z","M","Y","timegrids","timeinv","onset"),file=paste0("Data_GO_",M.label,"_",R.label,".RData"))
